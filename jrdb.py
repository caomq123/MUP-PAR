...
#
#     # save the global annotation dict
#     # np.save('/home/yanhaomin/project/panoramic_action/code/action_eval/out/re_annotated_w_personID/global_activity_label.npy',data)
#     return data
#
#
...
#
#
# def jrdb_read_dataset_new(path, seqs, num_actions, num_activities, num_social_activities, is_train):
...
#     #
#     #         # if this_seq_id == 16 and this_frame_id == 345:
#     #         #     print(this_person_id)
#     #         #     print(xy)
#     #         #     print(this_bbox)
#     #
#     #         if this_seq_id in seqs:
...
#     #         # if (is_train and int(frame_id) > int(NUM_FRAMES[seq_id] * 0.25)) or (
#     #         #         not is_train and int(frame_id) <= int(NUM_FRAMES[seq_id] * 0.25)):
...
#     if is_train:
#         data_all = np.load(
#             join(myjson["Datasets"]["JRDB"]['annotation_path'], 'train_annotation_all.npy'), allow_pickle=True)
#     else:
#         data_all = np.load(
#             join(myjson["Datasets"]["JRDB"]['annotation_path'], 'test_annotation_all.npy'), allow_pickle=True)
#
#     return data_all
#
#
...
#         self.anns = anns.item()         # self.anns = anns
#         self.frames = frames
...
#             img = np.array(img)                                         # 取数据集中的img, resize成(ow ,oh), 再转换成tensor
#
#             # H,W,3 -> 3,H,W
...
#             if len(temp_boxes) < self.num_boxes:                                      # 不足固定的框数的时候，用0补充
...
#
#             activities.append(self.anns[sid][src_fid]['group_activity_all'])         # 一张图仅有一个activities（global）
#
...
NUM_FRAMES = {0: 1728, 1: 580, 2: 1441, 3: 1009, 4: 1298, 5: 1448, 6: 864, 7: 467, 8: 1011, 9: 726, 10: 1730, 11: 874,
              12: 1156, 13: 725, 14: 1524, 15: 1091, 16: 460, 17: 1014, 18: 718,
              19: 853, 20: 1373, 21: 1737, 22: 868, 23: 873, 24: 432, 25: 505, 26: 1442}     # 一共含有27个视频--20个训练、7个测试
...
class JRDB_Dataset(data.Dataset):
    ...
    def __init__(self, num_actions, num_activities, num_social_activities, anns, frames, images_path, image_size,
                 feature_size, num_boxes=13, num_frames=10,
                 is_training=True, is_finetune=False, flag=True):
        self.anns = anns.item()         # self.anns = anns
        self.frames = frames
        self.images_path = images_path
        self.image_size = image_size
    ...
    def load_samples_sequence(self, select_frames):
        ...
        for i, (sid, src_fid, fid) in enumerate(select_frames):     # 一张一张图片来的
            this_image_path = self.images_path + '/' + seq_names[int(sid)] + '/' + str(fid).zfill(6) + ".jpg"
            # print(this_image_path)
            if os.path.exists(this_image_path):
                img = Image.open(this_image_path)
            ...
            img = transforms.functional.resize(img, self.image_size)
            img = np.array(img)                                         # 取数据集中的img, resize成(ow ,oh), 再转换成tensor
            # cv2.imwrite('primary_image.jpg', img)    在这里是可以正常显示图片的
            # exit()
        ...
        # print(bboxes_num.shape)
        # print(social_activity.shape)
        # print(person_id.shape)
        # print(social_group_id.shape)
        # exit()
