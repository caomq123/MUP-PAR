...
def prep_images(images):
    ...
    images = torch.sub(images, 0.5)    # tensor 相减？
    images = torch.mul(images, 2.0)    # tensor 相乘？
    ...
    if phase == 'Test':
        ...
        print_log(log_path,
                  'Global f1: %.2f%%,action f1: %.2f%%, social activity f1: %.2f%%, Overall f1: %.2f%%, Loss: %.5f, Using %.1f seconds' % (
                      ...
                      info['time']))
def show_epoch_info2(phase, log_path, info, cfg):
    print_log(log_path, '')
    if phase == 'Test':
        print_log(log_path, '====> %s at epoch #%d' % (phase, info['epoch']))
    else:
    ...
    if phase == 'Test':
        print_log(log_path,
                  'Global f1: %.2f%%,action f1: %.2f%%, Overall f1: %.2f%%, Loss: %.5f, Using %.1f seconds' % (
                      info['activities_f1'], info['actions_f1'],
                      info['overall_f1'],info['loss'],
                      info['time']))
        print_log(log_path,
                  'Group_Activity p: %.2f%%,action p: %.2f%%' % (
                      info['activities_p'], info['actions_p']))
        print_log(log_path,
                  'Group_Activity r: %.2f%%,action r: %.2f%%' % (
                      info['activities_r'], info['actions_r']))

    else:
        print_log(log_path,
                  'Global f1: %.2f%%,action f1: %.2f%%, Overall f1: %.2f%%, Loss: %.5f, Using %.1f seconds' % (
                      info['activities_f1'], info['actions_f1'], info['overall_f1'],info['loss'],
                      info['time']))
