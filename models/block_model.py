...
class Block(nn.Module):
    ...
    def predict_social_activity_score_train(self, social_group_id, boxes_states):
        ...
        social_group_id = social_group_id.reshape(-1)  # --9个[2, 2, 2, 6, 6, 1, 1, 5, 5]
        social_group_gt = create_P_matrix(np.array(social_group_id.cpu()))
        social_group_gt = generate_list(social_group_gt)
        social_group_gt.sort()   # [[0, 1, 2], [3, 4], [5, 6], [7, 8]]
        group_states_all = []
        # print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        # print(social_group_gt)
        for this_group in social_group_gt:      # [0, 1, 2]/[3, 4]/[5, 6]/[7, 8]
            group_states_list_this = []
            for person_id in this_group:        # 0/1/2
                group_states_list_this.append(boxes_states[person_id])
            group_states_this = torch.stack(group_states_list_this)   # [3, 1024]
            # group_states_this = group_states_this.mean(dim=0)
            print("Individual：")
            group_states_this = self.gcn_group(group_states_this[None, ...])   # GCN网络--[1, 1, 1024]  一组的聚成一个feature了吗！
    ...
    def predict_social_activity_score_test(self, relation_graph, boxes_states):
        ...
        # use clustering to get the groups   测试的时候不输入组的id，是通过聚类的方法进行分组的
    ...
    def forward(self, batch_data):
        ...
        images_in, boxes_in, bboxes_num_in, bboxes_social_group_id_in = batch_data    # 初始会提供组id吗？--训练的时候会的，测试的时候使用聚类
        ...
        outputs = self.backbone(images_in_flat)            # Inception-V3 backbone网络提取输入图片的特征outputs呢

        # Build multiscale features   构建多尺度特征
        features_multiscale = []
        ...
        for b in range(B):
            # 每个b中有一张image
            p_image = images_pri[b, :, :, :]  # [3, 480, 3760]
            p_image = np.array(p_image.detach().cpu().numpy())
            pp_image = p_image.transpose(1, 2, 0)
            cv2.imwrite('PPrimary_image.jpg', pp_image)
            N = bboxes_num_in[b][0]  # 9  N应该是一个image的9个person框才对
            boxes_features = boxes_features_all[b, :, :N, :].reshape(1, T * N, NFB)  # 1,T,N,NFB--[1, 9, 1024]
            boxes_positions = boxes_in[b, :, :N, :].reshape(T * N, 4)  # T*N, 4---[9, 4]
            bboxes_social_group_id = bboxes_social_group_id_in[b, :, :N, :].reshape(1, T * N)  # T*N, 4---[1, 9]

            # Node graph, for individual
            graph_boxes_features, relation_graph = self.gcn_ind(boxes_features, boxes_positions)
            # graph_boxes_features[1, 9, 1024]   relation_graph[1, 9, 9]   这是9个组还是9个人呐--9个人

            # graph attention module
            graph_boxes_features = graph_boxes_features.reshape(1, T * N, NFB)   # --[1, 9, 1024]
            # graph_boxes_features, adjacency_matrix = self.graph_attention(graph_boxes_features, relation_graph)

            # handle the relation graph
            relation_graph = relation_graph.reshape(relation_graph.shape[1], relation_graph.shape[1])  # --[9, 9]
            ...
            boxes_features = boxes_features.reshape(1, T * N, NFB)  # ---[1, 9, 1024]
            boxes_states = graph_boxes_features + boxes_features  # 1, T*N, NFG 顶点的特征是原特征加加权之后的特征
            boxes_states = self.dropout_global(boxes_states)   # ---[1, 9, 1024]
            NFS = NFG
            boxes_states = boxes_states.reshape(T * N, NFS)  # ---[9, 1024]

            # Predict social activities
            if self.training:
                boxes_states_for_social_activity = self.predict_social_activity_score_train(
                    bboxes_social_group_id, boxes_states.reshape(N * T, NFS))    # [4, 1, 1, 1024] 这个image里面是四个组嘞
            else:
            ...
            print("global1：")
            boxes_states_pooled = self.gcn_ind2global(boxes_states.reshape(1, T * N, NFS))  # GCN网络[9, 1024]-->[1, 1, 1024]
            boxes_states_pooled_1 = boxes_states_pooled.reshape(1, NFS)   # [1, 1024]

            print("global2：")
            group_states_pooled = self.gcn_group2global(boxes_states_for_social_activity.reshape(1, -1, NFS)).reshape(1,
                                                                                  NFS)
            # vis_image0 = boxes_states_pooled_1.detach().cpu().numpy()
            # cv2.imwrite('cam_image0.jpg', vis_image0)

            # [1, 1024]
            global_f = boxes_states_pooled_1 + group_states_pooled    # 9个人的特征gcn聚合成一个+四个组的特征gcn聚合成一个
            acty_score = self.FFN_activities(global_f)    # 7个分数(global的动作识别)

            bboxes_social_group_id = bboxes_social_group_id.reshape(-1)  # --9个[2, 2, 2, 6, 6, 1, 1, 5, 5]
            social_group_gt = create_P_matrix(np.array(bboxes_social_group_id.cpu()))
            social_group_gt = generate_list(social_group_gt)
            social_group_gt.sort()  # [[0, 1, 2], [3, 4], [5, 6], [7, 8]]
            DB = [[2, 17], [3, 4], [5, 7], [8, 9]]
            ...
            if social_group_gt == DB:
                print(actn_score.shape)
                print(actn_score)
                print(acty_score.shape)
                print(acty_score)
                print(social_acty_score.shape)
                print(social_acty_score)
                for this_group in social_group_gt:  # [0, 1, 2]/[3, 4]/[5, 6]/[7, 8]
                    for person_id in this_group:
                        print(boxes_positions[person_id])
                exit()
